...
public class Search extends AbstractPage {
    ...
    public boolean checkListAuthors(){
        if(listAuthors.size() == 0)return false;
        return true;
    }
    public boolean checkListArtworks(){
        if(listArtworks.size() == 0) return false;
        return true;
    }
    public boolean checkListPosts(){
        if(listPosts.size() == 0)return false;
        return true;
    }
    public boolean checkListIllustrations(){
        if(listIllustrations.size() == 0)return false;
        return true;
    }
    public boolean checkListCollections(){
        if(listCollections.size() == 0) return false;
        return true;
    }
